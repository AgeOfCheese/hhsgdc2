...
func _ready():
	...
	call_deferred("setup_phosphene_sprite")

func setup_phosphene_sprite():
	viewport = get_tree().current_scene.get_viewport()
	
	# Create the phosphene sprite and add it to the current scene
	phosphene_sprite = Sprite2D.new()
	phosphene_sprite.name = "PhospheneSprite"
	phosphene_sprite.z_index = 100  # Make sure it's on top
	phosphene_sprite.position = viewport.get_visible_rect().size / 2.0
		
	get_tree().current_scene.add_child(phosphene_sprite)
